...
contract Fund is RoleAware {
    ...
    // withdrawers role
    function withdraw(address withdrawalToken, address recipient, uint withdrawalAmount)
        external returns (bool) {
        require(isWithdrawer(msg.sender), "Contract not authorized to withdraw");
        return IERC20(withdrawalToken).transfer(recipient, withdrawalAmount);
    }

    // withdrawers role
    function withdrawETH(address recipient, uint withdrawalAmount)
        external {
